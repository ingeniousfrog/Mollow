...
const MF_E_TRANSFORM_NEED_MORE_INPUT: i32 = 0xC00D_6D72_u32 as i32;
...
unsafe fn activate_object(activate: *mut *mut c_void) -> Result<*mut c_void, BenchmarkError> {
    ...
    unsafe {
        ...
        if activate_object(*activate, &raw const IID_IMFTRANSFORM, &raw mut transform)
            != ERROR_SUCCESS
        {
            return Err(BenchmarkError::new(
                "media",
...
unsafe fn configure_input_type(transform: *mut c_void) -> Result<(), BenchmarkError> {
    // SAFETY: IMFMediaType is configured through valid COM pointers.
    unsafe {
        ...
        let _ = set_uint32(media_type, 0, 0x3436_3248);
...
unsafe fn create_sample(bitstream: &[u8]) -> Result<*mut c_void, BenchmarkError> {
    // SAFETY: IMF sample and media buffer are built through valid COM pointers.
    unsafe {
...
unsafe fn process_output(transform: *mut c_void) -> Result<(), BenchmarkError> {
    // SAFETY: IMFTransform::ProcessOutput is invoked through a valid COM pointer.
    unsafe {
...
unsafe fn vtable_fn<T>(object: *mut c_void, index: usize) -> Result<T, BenchmarkError> {
    // SAFETY: COM vtable entries are read from a valid object pointer.
    unsafe {
        if object.is_null() {
