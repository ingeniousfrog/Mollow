...
`mollow inspect --format terminal --lang zh-CN` 的终端输出示例（v0.1.3）：

### Ubuntu 云 GPU 服务器（阿里云 ECS）

可读 GPU 型号来自 `nvidia-smi`（`NVIDIA A10`）与 `pci.ids`（虚拟显示 `Cirrus Logic GD 5446`）：

![Ubuntu 云 GPU 服务器 inspect 输出](docs/images/inspect-ubuntu-gpu.png)

### macOS（Apple Silicon）

内置 GPU、VideoToolbox 媒体能力与开发工具版本：

![macOS inspect 输出](docs/images/inspect-macos.png)
