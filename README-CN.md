...
> **快速了解：** 不想通读 README？先看 **[Mollow 可视化导览](https://ingeniousfrog.github.io/Mollow/)** — 安装路径、项目结构、典型工作流的一页概览（类似简介 PPT）。
...
- [Mollow 能做什么](#mollow-能做什么)
- [inspect 示例输出](#inspect-示例输出)
- [整体流程](#整体流程)
- [安装](#安装)
- [命令与参数](#命令与参数)
- [基准配置档](#基准配置档)
...
| 能力域 | 说明 |
| --- | --- |
| **机器快照**（`inspect`） | 操作系统、CPU、内存（含内存条详情）、存储卷、GPU、媒体编解码、电源、温控、开发工具；可选 `--enrich` 离线硬件目录 |
...
### 快照 Schema（v4）— `inspect` 采集内容

| 组件 | 示例字段 |
| --- | --- |
| `system` | 系统名称/版本、内核、架构、主机名 |
| `cpu` | 型号、物理/逻辑核心数、指令集特性 |
| `memory` | 总/可用内存、交换区、内存条类型/频率（若可探测） |
| `storage` | 挂载点、卷容量、文件系统类型 |
| `gpu` | 设备名、厂商、图形 API |
| `media` | 后端、硬件编解码能力列表 |
| `power` | 交流/电池、电量、低功耗模式 |
| `thermal` | 温控状态、温度、传感器 |
| `runtimes` | rustc、cargo、git、node、python（若已安装） |
| `hardware_context` | 可选（`--enrich`）：目录规格、架构摘要、基准参考 |

详见 [硬件 enrichment 文档](docs/hardware-enrichment.md)。

---

## inspect 示例输出

`mollow inspect --format terminal --lang zh-CN` 的终端输出示例（v0.1.4）：
...
**v0.1.4 要点：** 快照/基准 Schema 升至 **v4**；可选 `--enrich` 离线硬件目录（CPU/GPU/内存规格、架构摘要、HTML 简化示意图、基准参考对比）；内存条探测（Linux DMI、macOS `system_profiler`）。**v0.1.3 要点：** Linux 上 GPU 显示可读型号（`nvidia-smi` / `pci.ids`）。**v0.1.2 要点：** 修复 Linux 上 `GLIBC_2.39 not found`（Ubuntu 20.04、阿里云 ECS 等）；安装脚本默认使用 musl 静态二进制。
...
**Windows PowerShell** — 重新运行 `install.ps1`，或 `.\install.ps1 -Version 0.1.4`（安装后重启终端）。
...
| 文档 | 内容 |
| --- | --- |
| [docs/hardware-enrichment.md](docs/hardware-enrichment.md) | 离线硬件目录、`--enrich`、schema v4 |
